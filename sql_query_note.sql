...
truncate tb_stock;
-- truncate tb_history;
select u.username, sum(t.payment) as totalpayment from tb_transactions t 
join tb_users u on t.userID = u.id group by u.id order by totalpayment  desc;
...
Select * from tb_products p join tb_productcat pc on p.id=pc.productID 
join tb_categories c on c.id = pc.categoryID where c.category = 'Global Product' order by p.last_update asc; 
...
select c.category, cc.category from categories c join categories cc on c.id=cc.parentId;

select c.id, c.category as categorychild, c.parentID, ca.category as categoryparent
from categories c
left join categories ca
on c.parentId = ca.id;

select distinct p.id, p.nama from products p -- distinc menghilangkan data yang berulang menjadi satu data saja 
...
WITH RECURSIVE category_path (id, category, path) AS
(
  SELECT id, category, category 
    FROM categories
    WHERE parentId IS NULL
  UNION ALL
  SELECT c.id, c.category, CONCAT(cp.path, ' > ', c.category)
